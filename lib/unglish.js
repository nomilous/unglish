...
module.exports.generate = function generate(length) {
  ...
  for (var i = 0; i < length && word.length != length; i++) {
    ...
    // alternate between consonant and vowel chains
...
module.exports._next = _next = function _next(word, length, i, type) {
  ...
  if (!_allowed(word, length, type, nextLength, nextChain, first, middle, last)) {
...
module.exports._allowed = _allowed = function _allowed(word, length, type, nextLength, nextChain, first, middle, last) {
  ...
  // 'q' cannot be second last letter
  if (nextChain[nextChain.length - 1] == 'q' && word.length + nextLength + 1 == length) return false;

  // no 'q' without following 'u'
  if (word[word.length - 1] == 'q' && nextChain[0] != 'u') return false;

  // 'q' requires two vowels to follow
  if (word[word.length - 1] == 'q' && nextLength < 2) return false;

  // no 'wk' without preceding 'a'
  if (nextChain == 'wk' && word[word.length - 1] != 'a') return false;

  // 'rc' without preceding 'a' or 'e' or 'o'
  if (nextChain == 'rc' &&
    !(word[word.length - 1] == 'a' || word[word.length - 1] == 'e' || word[word.length - 1] == 'o')) return false;
  ...
  if (first) {
    ...
    if (nextChain == 'pt') return false;
    if (nextChain == 'ly') return false;
    if (nextChain == 'gm') return false;
    ...
    if (nextChain == 'rr') return false;
    if (nextChain == 'pt') return false;
    if (nextChain == 'v') return false;
