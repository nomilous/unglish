...
module.exports.generateConfig = generateConfig = function generateConfig(inputFile, outputFile) {
  ...
  var chains = _loadChains(inputFile);

  _saveChains(outputFile, chains);

};

module.exports._loadChains = _loadChains = function loadChains(inputFile) {
  ...
  vowels = _scanWords(words, 'vowels');
  consonants = _scanWords(words, 'consonants');
...
module.exports._scanWords = _scanWords = function scanWords(words, type) {
  ...
  function handleChain(chain, front, back) {
    if (type == 'vowels' && chain.length > 2) {
      return;
    }

    if (type == 'consonants' && chain.length > 3) {
      return;
    }
...
module.exports._saveChains = _saveChains = function saveChains(outputFile, chains) {
